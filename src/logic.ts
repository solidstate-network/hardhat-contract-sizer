...
export async function sizeContracts(
  ...
  // get the solc version used for each artifact, indexed by build info id
    ...
    artifacts.map(async ({ buildInfoId }) => {
      if (buildInfoId) {
        ...
        buildInfoSolcSettings[buildInfoId] = {
          solcVersion: json.solcVersion ?? 'unknown',
          optimizer: json.input?.settings?.optimizer?.enabled ?? false,
          runs: json.input?.settings?.optimizer?.runs ?? 0,
        };
      }
    }),
  );
  ...
  const outputData: Output = artifacts.map((artifact) => {
    ...
    return {
      ...
      // TODO: build info id might be undefined for contracts not generated by Hardhat
      solcSettings: buildInfoSolcSettings[buildInfoId!],
    };
  });
  ...
  for (const key in outputDataBySolcSettings) {
    ...
    table.push([
      {
        content: chalk.gray(`Solc version: ${solcSettings.solcVersion}`),
      },
      {
        content: chalk.gray(`Optimizer enabled: ${solcSettings.optimizer}`),
      },
      {
        content: chalk.gray(`Runs: ${solcSettings.runs}`),
      },
    ]);
